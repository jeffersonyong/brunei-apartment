...
- **The unit type's `car_allowance` is shown, and still not enforced** (§7.1; the showing half added 17 September 2026). A family that turns up in three cars for a two-car unit is a fact Security needs recorded, not a booking to refuse; whether that is chargeable or capacity-limited waits on **R3** in the [open-questions register](open-questions.md), which asks how many bays the property actually has. What changed is that the form no longer keeps the allowance to itself: every form that collects plates against a unit — the public stay form, and the desk's create and amend — names what the unit includes under the rows, and warns once the plates entered are past it. The rows stay addable, because §12.5 makes plate lookup the guard's primary path and a plate the form refused is a car nobody can match at the gate. Day passes are exempt: a pass carries no unit and so no allowance, and visitor parking is the other half of R3.
