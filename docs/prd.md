...
| **Security** | Phone browser | The front desk at the gate: see today's arrivals and departures, check guests in and out by QR or vehicle registration, admit day passes, and take cash a guest still owes |
...
**[A] Checking a guest in and checking one out are two permissions, `booking.check_in` and `booking.check_out`** (13 September 2026, answering the first half of [N11](open-questions.md) — Jeff's decision; who holds them is Jason's since [N54](open-questions.md), below). From the deposits slice until then both moves borrowed `booking.amend`, which meant nobody but the desk could check anybody in, and that granting it to a guard would also have let them change a booking's dates, unit and guest. Each move is now held by the role whose job it is: **the guard checks guests in and out**, because he hands the keys over and takes them back; **Housekeeping checks a guest out when it finds the unit empty** with the keys left in it; and Front Office and Admin hold both. Neither handles money — `check_in_booking()` refuses a booking whose deposit is not held in full (§11, §12). `booking.amend` keeps its own meaning: editing a booking.

**[C] The guard is the front desk** (Jason, 14 September 2026, answering [N54](open-questions.md)): *"security guard and front desk are the same people. the security guard is the one that hands customers the keys. Therefore they can check people in."* So Security holds `booking.check_in`, and `booking.check_out` too — the guard checks short-stay guests out from the gate when the keys come back. That reverses Jeff's earlier call, which had taken check-in back from the guard on the assumption that the keys were handed over at a counter. Admitting a day pass stays its own permission, `day_pass.admit`, held by Security, Front Office and Admin — a different move rather than a check-in by another name, because admitting closes the pass (§9.2, §12). **[C] The guard cannot make a booking on the spot** — he calls the back office first — so Security holds no `booking.create` or `booking.amend`, and the office can book a guest waiting at the barrier for the guard to collect (§9.1). **[C] The guard records the cash he is handed**, so Security holds `payment.record_cash` (§10.5). **[A] What stays withheld is verification.** The guard never says a bank transfer landed: recording a transfer as money *already seen* — the rest of a short deposit, or what a guest owed beyond their deposit — takes `payment.verify` as well as `payment.record_cash`, so the principle at the foot of this section still holds.
...
| **Security** | `booking.check_in`, `booking.check_out`, `day_pass.admit`, `payment.record_cash` — the gate (§12): checking guests in and out, admitting day passes, and taking cash a guest still owes — and a read-only booking view. No booking creation, no document access, and no payment verification. |
| **Finance** | `payment.verify`, `deposit.approve_release`, `charge.waive`, `report.view`, read-only booking view |

**[A] What "(status only)" means concretely**, settled when the units screens were built. `unit.manage` opens the units board and the two service actions — taking a unit out of service and returning it — which is exactly the cleaner's job, and **marking a unit ready after a stay** (§6.4), which is the same kind of operational statement. Two neighbouring things are deliberately *not* on it:
...
**As built (capabilities A9 and A10, 16 September 2026).** Two more screens, and between them they close the customer surface's self-service loop. `/find-booking` takes the reference from a transfer and the phone number a booking was made with, and opens that booking — **including a booking taken at the desk, which is given a link the first time somebody asks for one.** Until the confirmation email is switched on (register N42) this is the only route back to a booking whose link has been lost, and it is the only route at all for a walk-in guest. **Three things now point at it**, because a page nobody can find is not a route: a line in the landing page's own fine print where somebody reading how booking works would think of it, a `Find booking` button in the public header beside the `Book` menu (hairline, not a second lagoon fill — the surface gets one; both at every width since 14 September 2026, when `Book` replaced `Book a stay` and opens onto a day pass or a stay), and a footer line on both confirmation emails carrying the link. The email one is the one that matters for a desk booking, which has no private link to lose, and like the rest of A8 it does nothing until N42. `/faq` answers what guests ask before they book, with every rate, time, facility and bank account read live from Property settings rather than written into the copy — the drift the landing page's own day-pass line already has is the thing that rule exists to prevent.

**The FAQ leaves off what it cannot answer** (Jeff, 14 September 2026). Eight questions a guest would obviously ask are open items in the register, and until then each carried a visible "to confirm" marker beside the part of the answer that was confirmed. Writing the page is what raised **N44** (whether a stay includes facility use), **N45** (whether cancelling early costs less) and **N46** (house rules), none of which any document answered. Once staff could write the FAQs themselves (below), a marker on a public page became a gap somebody can close from the portal the day the answer is agreed, so those eight are not on the site; the questions stay open in the register. **[A]**
...
**[C] Its photographs are staff-managed** (capability F7; proposed 10 September 2026, agreed with the client and built). A unit type gets repainted, a facility reopens, somebody commissions a better photo shoot — and under any arrangement where the images ship with the code, each of those is a developer deploy. That is the same argument §7.2 already accepted for facility inclusion and §7.1 for unit naming, applied to the one part of the product a customer actually looks at first.

**What is managed is exactly what the landing page shows**: the front-page photo, one per day-pass facility card and per unit-type card (found by the slug a rename never moves, and both lists read from Property settings since 17 September 2026, so a facility ticked into the pass or a unit type added gets its place the same day), and the four "Follow along" tiles — one current photo each. Staff add, replace or remove a photo, write its description, and choose which part stays in view when the site crops it; removing one brings the placeholder back. **The description is the image's alt text, not the card's copy**: the words beside each photo stay in the landing page and stay confirmed facts only, so no staff-typed sentence can advertise something the business has not decided. What it costs architecturally is §8's one departure — a **public** bucket, `site-images`, with no retention period — as built in [architecture.md §8.3](architecture.md). Changing the photos is its own permission; see §4.

**[A] Its questions are staff-managed too** (capability F9; proposed 14 September 2026 by Jeff, built, not yet agreed with the client). The questions change as the business does, and while the FAQ was copy in code each change was a developer deploy — F7's argument, applied to the words. Staff add, reword, reorder and remove questions under five fixed topics from *Admin → Website FAQs*, and choose up to six to show on the landing page as well, in a section before its closing band that is left out when none is chosen. The page is titled **FAQs**, and each question opens to show its answer. **No figure is typed into an answer**, which keeps the rule the page was built on: an answer names a live figure in braces — `{security deposit}`, `{nightly rates}`, `{bank accounts}` — and the site fills it in from Property settings as the page renders, so a rate changed in settings is the rate every answer quotes. A name the site cannot fill in is refused on save; an amount typed by hand is pointed out but allowed. A question's address, `/faq#<slug>`, is fixed when it is added, so rewording it never breaks a link staff have sent. A removed question is deleted, and its audit event keeps what it said. Every property starts with the twenty questions the hand-written page answered in full, six of them on the landing page. Changing them is its own permission; see §4. As built in [architecture.md §8.4](architecture.md).
...
Purpose-built single screens. Security: the gate — today's arrivals and departures, checking stays in and out, admitting day passes, and taking cash a guest owes (§12). Housekeeping: today's checkouts, inspection, unit ready toggle. No app installation.

**As built — the gate (capabilities D1, D2, D4 and check-in).** `/field/arrivals` is the guard's screen, and a guard signs in straight to it: somebody whose every permission is one the field screens use, and who has a field screen to work, lands on `/field` rather than the portal; the desk, which can also check guests in, lands on the portal as before. One page carries the whole day — stays open over today, today's day passes, and guests already checked in — and the guard filters it by plate, name or reference on the phone, so matching a car needs no request. A car that is not on the list is found by searching every open booking. What the screen decides about each car is §12's as-built block. **[A]**

**It is the Gate** ([N54](open-questions.md)): the same route, `/field/arrivals`, which guards have bookmarked, with guests due out today in a section of their own and — on the phone of a guard who may take it — the cash a guest still owes on their card.

**As built — housekeeping (capabilities C1–C3).** `/field/departures` is the cleaner's screen, and Housekeeping signs in straight to it. It answers to `inspection.record`, the one step of three only housekeeping takes, so the desk — which also checks guests out and manages units — is not handed a cleaning list. It lists every unit with a turnover under way, read from the same facts as the units board, in three sections: **Leaving today** — a guest due out today or overdue and still checked in, where "Guest has left" checks them out (§4); **To inspect** — checked out and not inspected, where the inspection and its photographs are a page of their own (§11); and **Being cleaned** — inspected, where "Mark ready" finishes the turnover (§6.4). A unit somebody arrives in today comes first. Each card shows the unit's standing note and the office's notes for housekeeping on that stay, read-only, and never an internal note (§9.7). Each step answers to its own permission — `booking.check_out`, `inspection.record`, `unit.manage` — so a role holding fewer sees a sentence naming who takes the step rather than a button that refuses. Somebody holding both field jobs switches between them from a bar under the header. **[A]**

**[A] A guest is never checked out from the phone before their last day.** "Guest has left" appears on the day a guest is due out, not before, and the action checks again when it is pressed. A phone cannot tell a guest who has gone from one who is out for the day; a guest who really has left early is checked out at the desk, which can ask them.
...
**How each state is actually held** — settled by the units slice (B8–B9) and completed by the housekeeping slice (C1–C3), and worth being explicit about because so little of it is stored:
...
| `awaiting_inspection`, `cleaning` | **Derived** from the unit's last stay (capabilities C2–C3). A guest who has checked out with no inspection recorded leaves the unit *awaiting inspection*; once it is inspected it is *cleaning*; once housekeeping marks it ready it reads from whatever covers the day. The one new stored fact is **when a unit was made ready** — written once, on the stay's inspection. |

**[A] Ready is a status on the units board and the cleaner's phone, and nothing else** (Jeff, 13 September 2026). Availability, selling and check-in never read it: a unit nobody has marked ready can still be booked and checked into, exactly as a unit nobody had inspected could before. Whether it should hold back the next check-in is put to Jason as [N53](open-questions.md). One consequence worth stating: check-out never shortens a stay, so a guest who leaves early keeps their unused nights unsellable — and once the unit is marked ready, the board calls it available while availability still does not offer those nights.

**[A] How a turnover reads on the board.** A guest still checked in makes the unit *occupied* whatever the dates say — on their last day, and past it — where the board used to call that unit available. On a changeover day *awaiting inspection* and *cleaning* show ahead of the next guest's *booked*: the next guest is coming either way, and what the board has to say is that the unit is not yet clean. **Mark ready requires an inspection and is written once**, with no undo in v1, and it is refused once the next guest has checked in. **Turnovers are kept from the day this shipped**: a stay that ended before then never reads as awaiting inspection, rather than the board opening on every past stay nobody inspected in the system.
...
**[A] The tick is what the public site sells** (17 September 2026). The landing page's day-pass section listed three hard-coded facilities and, under them, a line of fine print naming what the pass left *out*. Both are gone. The section now renders a card per included facility, in the order the settings screen lists them, which is the same answer `/day-pass` already stated before taking a booking — so ticking a facility sells it on the front page, un-ticking one stops the page advertising a pass that no longer admits it, and the unresolved facilities are governed by their tick rather than by a list in the code that nobody would think to edit when Jason answers [C1](open-questions.md). **Nothing states what is excluded.** A page selling a pass that names four facilities a visitor will not get argues against itself, and the exclusion was never a fact a customer had asked for. Photo places follow the same ticks (capability F7), so a newly included facility has somewhere to hang a photograph the same day. **[A] A card can be switched off without un-ticking** (18 September 2026, Jeff; not yet agreed with the client): the Photos tab's *Show on the front page* leaves one facility's card off the landing page — for a facility with no photograph, or one not worth advertising — while `/day-pass` keeps listing it, because the pass still admits it. From a tablet up the cards are one row that scrolls sideways under a pair of arrows, rather than a grid that leaves a lone card on its last line.
...
**[A] One exception to "never held against nothing": a guest already at the barrier** ([N54](open-questions.md); kept today-only by Jeff, [N58](open-questions.md)). The guard cannot make a booking, so he calls the office, and the office can book the guest in front of him with **nothing taken** — held, with no deposit and no payment — for the guard to take the deposit and then the stay in cash when the car comes through (§12). It is refused for a stay not starting today in Brunei and beside a waived deposit: held for a later date it would be the unpaid reservation this section rules out, and a waived booking has nothing to secure it but the stay. The deposit still confirms it — the guard's cash for it is what does. Nothing expires the hold (§9.3), so the office cancels a booking whose guest drives off.
...
**As built (15 September 2026).** The desk form takes the deposit as the booking is made, and asks which of the customer's two answers (§10.3) the guest is giving — *the deposit only*, or *the deposit and the stay*. That is this paragraph's distinction made into one control rather than left implicit: the walk-in at the counter pays everything, the regular ringing ahead pays the BND 100 and settles on arrival, and both are one form. ~~The deposit alone is the **default**, for the reason it is the default on the customer's own page.~~ The figure crossing the counter is on the submit button either way.

**The default is *the deposit and the stay*** (Jeff). The old default's reasoning cited the customer's own page, and that page had been reversed to paying in full the day before (§10.3) — so the desk was following a rule that no longer existed anywhere. The desk's case is the stronger of the two on its own terms: §9.4's whole subject is the walk-in at the counter, who is paying for the stay now, and opening on *deposit only* recorded a booking carrying a balance nobody had agreed to defer. What guarded against over-collecting in cash still does — the figure is on the button — and **the summary beside the form now itemises what makes it up** rather than describing it in a sentence: a *Taking now* block listing the stay and the refundable deposit under the booking total, adding up to the button's figure. The booking total above it stays the stay alone, because §11 keeps the deposit a liability rather than revenue and merging them is the one place in the product the two could be confused (§10.3). **[A]**

**A third way to pay: at the gate** (capability B17). For a stay starting today with no deposit waived, the method control also offers *At the gate — the guard takes it*: the booking is created held with nothing taken, the form hides the *Paying now* question and the itemised block because nothing crosses the counter, and the button reads *Create & hold for the gate*. The confirmation says the booking is held and what the guard will take. §9.1 records why it is today-only, and `create_walk_in_booking()` refuses any other date, or a waiver, itself. **[A]**

**Changing the unit type no longer needs a second click, and cannot go stale**. The type filter sat in the availability form, so choosing a different one changed nothing until somebody also pressed *Check availability* — and a clerk who changed it and carried straight on was filling in a form about the old type. The stale state was invisible and the booking it produced was wrong. Narrowing is a question about a list already in hand, so it moved into the form: the page reads every unit free for the dates and the island filters them, which reprices instantly and — unlike the round trip that was the alternative fix — keeps the guest's name and number, because nothing navigates. **[A]**
...
### As built (capabilities B3 and B16)

All three bullets above are built, and so is the no-show itself, which nothing could record before.

**Cancelling and marking a no-show settle the deposit as they close the booking, in one transaction.** `close_booking()` is the only way into either status — `transition_booking()` refuses both events — so no route closes a booking and leaves its deposit on the ledger. What happens is decided from the deposit row under its lock:

- **Held** (collected, not released): a no-show keeps it; a cancellation keeps it, or gives it back.
- **Promised and never verified**: nothing is held, so nothing is kept. The promise lapses: it leaves the payments queue, and verifying it afterwards is refused. If the money arrives anyway it is given back outside the system.
- **Waived, quoting nothing, or never taken**: nothing happens, and the dialog says which.

**[A] Keeping is the default; giving it back is the desk's choice, under `booking.cancel`.** The rule above is about the *guest's* cancellation. A booking made in error, or cancelled by the property, is not that, and without a way to say so the guest's BND 100 would become revenue with no way back. The cancel dialog offers both with keep selected, and the reason already required is what the choice is read against. A deposit given back is recorded as a release written at the close — the release arithmetic, no inspection — with a `deposit.returned` event. [N49](open-questions.md).

**Kept is three columns on the deposit** — when, by whom, how much — rather than a status: the when is revenue's date, the who is the audit's answer, and the stage stays derived (§11, where `forfeited` shows as *Kept*). What is kept is **what was held**, which for a deposit that arrived short (§11, [N48](open-questions.md)) is less than the quote. A kept deposit is final: a trigger refuses any top-up, release or charge against it, and refuses money — or a charge — arriving on any booking that closed without a stay.

**[A] A no-show keeps the deposit without asking, is recorded from the arrival day, and releases the unit.** Not before the day the guest was due — they have not failed to arrive yet — and under `booking.cancel`, because it closes a booking and keeps its deposit exactly as a cancellation does. No reason is asked for. **The unit goes back on sale for the nights they did not use**: `no_show` joins `expired` and `cancelled` in the exclusion constraint's `where` and every reader that repeats it (architecture.md §5.2). Making the no-show reachable without that would have left the unit unsellable for the whole booking and — a no-show being terminal — refused the new booking a late-arriving guest would need. [N50](open-questions.md).

**A kept deposit is revenue on the day it was kept**, in its booking's stream — [N32](open-questions.md)'s standing assumption, built on and still open. It has its own *Kept deposits* column in revenue by stream (§14) and sits in no method column, so Cash still agrees with the cash-up.

**Money paid for the stay is unchanged**: a cancellation refunds it outside the system (architecture.md §6.4), and the dialog says so.

**A deposit still held on a booking cancelled before this was built is left as it was** — on the ledger as *Held before arrival*, with nothing offered to settle it. It is not back-filled, because choosing keep or return for it now would be this slice making the desk's decision for a cancellation it never saw.
...
**[C] The party can change after a booking is sold, checked in included, and on a day pass** (Jason's team, 19 September 2026; the pricing rule Jeff's, the same day — capability B18). Guests arrive with more people than they booked, sometimes to avoid the extra-person charge, or with fewer, and the guard is often the one who checks them in — late, with the office shut — so the office catches up after check-in. A **Change** beside the Party line on the booking changes the party on any open booking under `booking.amend`; a closed one takes no more money and is refused. It is narrower than an amendment, and that is what lets it reach a stay already begun:

- **[A] On a stay only the party's lines move.** Every line the stay was sold with keeps its price; the extra-person line (§8.2) is worked out again for **every night of the booking**, at the rate in force now, and a discount is taken again off the new subtotal. Every night, because the extra people usually arrived with the party and the office changes it the next morning — charging from "today" would miss the night they came. A fixed discount the smaller booking is no longer worth is refused, not clipped. This answers the party half of [N12](open-questions.md); dates, unit and late hours on a checked-in stay stay blocked, and the pricing question for those is unchanged.
- **[A] A day pass is priced whole again** through the engine that sold it (§8.1), and a growing pass is checked against the day's capacity under the same per-date lock the sale takes.
- **The difference is the balance** (§10.7). More people: the guest owes it, and the gate card offers it as cash to take. Fewer: the Money card shows the booking overpaid, which is a refund settled with the guest outside the system, as an amendment's reduction always was.
...
**[A] Money is not moved by either action — but an amendment now records what it left owing.** A cancellation calculates no refund. **It does settle the security deposit** — kept, or given back where the desk chooses — because a kept deposit is money whose owner has changed, and leaving it on the ledger was the gap (§9.5's as-built block). An amendment still moves no money, but the difference it creates is no longer only a sentence on screen: the booking carries it as an outstanding balance and it can be settled in cash or by bank transfer from the booking itself (§10.7). A price *reduction* is unchanged — that is a refund, and refunds are settled outside the system. When this was written it was a direct consequence of N5 being open — the platform could not state a forfeiture policy it had not been given; N5 has since been answered and the forfeiture built (§9.5). Leaving refunds to a person is also consistent with architecture.md §6.4, where a v1 refund is a recorded instruction executed by a person in a banking app, never an automated movement.
...
**[A] A note about the *unit* rather than the stay is the unit's own, and the cleaner sees both.** "The shower door sticks" outlives every booking, so it is a single note on the unit (capability B14, §6.4) rather than a note on whoever was staying. **The housekeeping phone screen shows the unit's note and the stay's notes marked for housekeeping, read-only, and never an internal note** — the first half of [N18](open-questions.md), Jeff's decision on 13 September 2026, built with C1–C3. The internal audience is filtered out in the query that reads notes, not by the screen.
...
**The customer says when they have paid** (steps 3–4), and the booking is created before that. Creating it holds the unit; pressing *I have made the transfer* moves it to `awaiting_payment_verification` and raises the pending row. That is what `payment.created_at` was always meant to measure — the queue’s waiting column is how long somebody has been left waiting, not how long they spent on a form.

**Step 3 is an upload** (capability A6). The order is the one the sentence above sets: the customer says they have transferred, *then* sends the slip — because until the button is pressed there is no deposit and no payment row for the slip to be evidence of. The upload is offered and never required; a booking refused for want of a screenshot would be a booking lost for a file the bank app makes unnecessary (§10.4).

**What is raised depends on what the customer chose**, and both answers are his own (§9.1: *the deposit only, or the full amount with the deposit*). A short stay quoting a deposit offers the two on the instructions page — **paying in full is the default** (Jeff), reversing the earlier reading that the smaller commitment is the kinder default: a booking settled at the point of booking is one the desk never has to collect on arrival, and leaving only the deposit is a choice the guest should make rather than one the form makes for them — and raises a pending deposit, plus a pending payment for the stay when they choose to settle it now. Anything else has nothing to defer and is simply paid for.
...
**One button per row, and the escape hatch moved inside it** (capability B6). The queue offered a payment two actions — *Confirm* and *Match manually* — and a promised deposit only one, which was the asymmetry that exposed the problem: the deposit's missing hatch was justified on the grounds that a deposit is raised against one booking by the customer and so has nothing to match to, and a pending **payment** is raised by exactly the same button on exactly the same page. The reasoning never told the two apart. The guest who forgets to quote `PV-4821` on a BND 100 deposit is the same guest.

The fix is one door for both. A single *Confirm* dialog takes the amount, the reference as it appeared, and an optional **note** — and if the clerk clears the reference, the note becomes required, because with nothing quoted it is the only thing identifying the transfer. That is this section's escape hatch, in one field instead of the four the separate dialog asked for: a clerk who wants to record "sent by John's wife at 9pm, BIBD" writes that sentence rather than splitting it across a sender, a statement date and a justification. **The absence is the flag** — the same rule §10.4's amount override already used, where the discrepancy reveals the reason field rather than a door being chosen in advance by somebody who has not yet opened their bank app.

Nothing is lost from the record. `match_kind` is **derived** from whether a reference was kept, so a hand match is still flagged in the data, still carries a required reason, and still writes its own audit verb — and `deposit` gained the same two columns and the same constraint, so a deposit matched by hand now leaves the trail a payment always did. What is dropped is the *collection* of a separate sender and statement date, which the note subsumes; the rows already carrying them keep them. **[A]**

**The queue says what each row is for.** A column of badges — *Stay* or *Security deposit* — sits beside the guest, replacing a gray caption under their name that a scanning eye skipped. §10.4's list of columns was written before N29 made a booking secured by its deposit, so the queue it describes held one kind of row; it holds two, and which one a row is decides what confirming it does to the booking.

**The slip arrived with the documents slice** (7 September 2026, capability B10), and the delta against scope-of-capabilities.md B4 is closed. The queue's cell reads *On file* or *None*, and a slip is attached from the booking. Two things are deliberately unchanged by it: the bank app is still the check — the slip is filed under the payment as a record rather than presented as something to approve against — and **a slip belongs to a bank transfer** [A], because cash was counted at the desk and has no slip to send. **Customer-facing upload arrived** (capability A6): the guest sends the slip from their own booking link, and the desk keeps its control for the guest who sends it over WhatsApp anyway. The two do not fight — a guest's upload replaces their own and never one a clerk filed, and where a clerk has already filed one the guest is told it is on file. The bank app is still the check, and a slip is still evidence.
...
**[C] The guard collects cash too** (Jason, 14 September 2026, [N54](open-questions.md)): *"Sometimes guard receives pending cash payments so he needs to record this."* Security holds `payment.record_cash`, and the gate records what a guest still owes through the desk's own writers — the security deposit first (cash against a transfer promised and never sent fulfils the promise), then the rest of a short deposit, then the stay, or a day pass. Who and when are the guard's session and the moment; the amount rule is this section's, typed reason and all; and the cash-up counts his takings with the desk's. **[A]** Nothing is taken while a transfer for the same money waits to be checked, because it may already be in the bank and a recorded payment cannot be withdrawn; and nothing is taken once a stay is checked out, because a closed booking takes no payment — so the gate asks for the stay before the keys come back, and a guest who leaves still owing is the office's to deal with outside the system ([N57](open-questions.md), Jeff). How the guard's notes reach the office is the business's own arrangement, not something the system records ([N56](open-questions.md), Jeff).
...
**[A] The guard takes the keys back, not the deposit** ([N54](open-questions.md)). Checking a guest out at the gate moves the unit to inspection exactly as the office's check-out does, and the deposit follows the process above — inspection, charges, approval — before anything is handed back. The guard never hands the BND 100 back: the office processes it in the few days after check-out ([N55](open-questions.md), Jeff).
...
**It takes `payment.record_cash` and mints no new permission.** Taking money at the counter is one job, and this section has twice taken that position already — verifying a deposit rides `payment.verify`, and the excess above one is settled by whoever may record a payment. **[A]** One qualification, when the guard came to hold `payment.record_cash` ([N54](open-questions.md)): a top-up — or a settled excess — recorded as a *transfer* is a claim about the bank rather than money counted, so it also takes `payment.verify` (§4).
...
**There is a slip on a deposit** (capability A6, [N39](open-questions.md) answered). It used to have nowhere to live: a document hung off a payment id (architecture.md §8.1) and a deposit is not a payment, so the queue's cell carried a sentence saying so. `document` now carries a nullable `deposit_id` beside its `payment_id`, and a slip must hold **exactly one** of the two.

**It is not a fifth kind, and that is the decision worth recording.** A deposit slip is a transfer slip — the same `payment-slips` bucket, the same seven-year accounting clock, the same `booking.view` to open, the same `payment.verify` to attach. A new kind would have minted a bucket, a retention row and a label for a file identical in every respect but its pointer, and it would have fallen out of `accounting_pack_changed_at`, which asks for `kind in ('identity', 'payment_slip')` — so every pack would have gone on reporting itself current after a deposit slip was added to it. **Nothing here makes a deposit a payment:** `paid_cents`, the balance, the revenue report and the cash-up are untouched, because what moved was a document's pointer and not any money. **[A]**
...
**[A] A photograph is made smaller on the phone before it is sent** (with the housekeeping phone screen). A camera original is larger than the 4 MB a file may be (§13), so the browser redraws each one at no more than 2400px on its long edge as a JPEG — the step the public site's photographs already take — one at a time, and one that fails stays on the list to send again. That strips all EXIF: the GPS position of a photograph taken inside somebody's home goes, and so does the time the camera recorded. **What dates the evidence is the time the file reached the system and who sent it**, both written by the server rather than by the phone.
...
**The ledger gains a stage, because most deposits now belong to guests who have not arrived.** `secured` — *held before arrival* — sits between `awaiting_verification` and `in_house`. Reading a deposit taken at booking as "guest in stay" was the ledger assuming a deposit could only exist because somebody had checked in. A deposit against a booking that was cancelled or never turned up read `secured` too, as the least wrong stage available while the forfeiture was unbuilt. **It no longer can** — closing a booking settles its deposit as `forfeited` (*Kept*) or `released`, and a promise that lapsed reads `lapsed` (*Never received*); see §9.5's as-built block. Only a row cancelled before that still reads `secured`. **[A]**
...
### As built (capability B16) — a deposit that arrived short

The slice above made the deposit the thing that secures a booking. What it did not ask is what happens when only part of it turns up — a guest who sends BND 50 now and means to send the rest on Friday, or a bank that shaves a transfer fee off the top.

`verify_deposit()` accepted a figure other than the quote with a written reason and **overwrote** what was held with it, so confirming BND 50 against a BND 100 quote recorded 50, **confirmed the booking**, emailed the guest, and closed every door behind it: verifying again was refused as `already_collected`, recording again as `already_recorded`, and check-in saw a collected deposit and took nothing. The shortfall survived only as a `deposit.amount_overridden` event nobody reads at the door. The guest checked in against half a deposit and the release handed back the 50.

**A deposit short of the quote secures nothing. [A]** §11 already held that `booking.security_deposit_cents` is the quote and the deposit row is what was taken, and that the two can differ; what was missing is that nothing acted on the difference. `booking_deposit_is_secured()` now compares them, which carries the rule to `verify_payment()`, `record_cash_payment()` and `check_in_booking()` without any of them changing. `verify_deposit()` is the exception that had to be taught directly — it never asked that function, it applied the status pair it was handed, and left alone it would have confirmed a booking this rule then refuses at the door. A short deposit is **collected** either way: the money did arrive, the property owes it back, and refusing to record it would leave a clerk with a bank line and nowhere to put it. What stops is the inference that collecting it secured anything.

**The desk tops it up, on the row that already exists. [A]** One deposit per booking is the constraint that makes "what do we hold" answerable, so `top_up_booking_deposit()` adds to the figure rather than raising a second row. It is money already seen — cash counted, or a transfer the clerk has just read in the bank — and it never joins the verification queue, because a queue row is a promise and this row stopped being one when somebody collected it. It refuses to overshoot the shortfall: more than whole is money to refund, and a refund is still [N5](open-questions.md). The booking moves only when the deposit comes whole, decided under the row lock rather than from the pair the caller offered, so a part payment of a shortfall records the money and moves nothing.

**This is not part payments** ([N16](open-questions.md), scope X11). Nothing offers a guest the choice of paying half a deposit, any more than half a stay. This is the product coping with money that arrived wrong, which is a different thing from a policy it declines to offer.

**The shortfall is derived, never stored, and measured against the quote read live.** So an amendment that reprices a booking's deposit upward makes a deposit that was whole read as short, and check-in refuses it until the difference is collected. That is the honest answer — the money genuinely is not all there — and it is accepted rather than solved: the alternative is a second "collected against" column and two meanings of *short* on screen. A deposit that came up short may still be **released**: you give back what you actually hold less charges, and refusing the release would trap the guest's money for good.

**It is visible wherever the deposit is** — a `Short` badge beside the stage on the booking's Money card, the deposits ledger and the deposit's own screen, *Quoted* and *Short* rows in the figure table, the gap named in the check-in dialog, and a *Deposit quoted* line on a printed statement where the two differ. The customer's own page says what arrived and what is still needed, because the booking no longer confirms and the page would otherwise go on saying it was checking for a transfer somebody had already checked.

**The cash-up's deposit line is assembled from arrivals rather than from the row (§14).** A top-up moves what is held but neither the row's method nor its collection date, which broke that line twice over: cash added to a deposit that first arrived by transfer went into the drawer and into no figure, and cash added to a deposit collected last Monday moved **Monday's** total, which somebody may already have counted and signed. The reader now asks what arrived on each day — a deposit's own figure less everything added since, plus the cash top-ups taken that day — which the trail records exactly. Deposits stay out of the takings total; what changed is that the line beside it is true. **[A]**
...
6. The guard's screen displays **payment status**, so an unpaid arrival is flagged rather than waved through. **The guard does not verify a transfer.** ~~The guard does not confirm payment.~~ **Revised 14 September 2026 by the client** ([N54](open-questions.md)): the guard takes the cash he is handed and records it (§10.5); saying a bank transfer arrived stays with the office.
...
**Check-in takes no money, and that is the whole of what it does to a booking.** It moves the stay to `checked_in` and nothing else. The security deposit arrived when the booking was made (§9.1, §11), so a guest at the door already has one held — and a booking whose quoted deposit is *not* held **in full** is refused rather than made an occasion to collect it, with the way out named on screen. **[A]**

The refusal names which of three it is, because the way out differs: nothing was ever taken, a transfer is promised and unverified, or a deposit arrived **short** of the quote (§11). The third is the one that must not be sent to the payments queue — that transfer was verified, and looking for a promise nobody made is how a shortfall goes uncollected a second time; it is topped up from the booking. A booking that was never confirmed because its deposit came up short is stopped a layer earlier, by the state machine, which has no legal move from `awaiting_payment_verification` to the door at all.

This sharpens requirement 6 rather than changing it. The guard's screen still shows payment status so an unpaid arrival is flagged; what is now also true is that an arrival with no deposit cannot be checked in at all, by anybody, until one is recorded — by the office, or by the guard in cash ([N54](open-questions.md)). The stay's own balance is unaffected — a deposit-secured guest owes the whole stay on arrival and settles it at the desk or the gate (§10.5, §10.7), which is a payment against the booking and not a condition of checking in.

### As built — the gate (the guard as the front desk)

**All eight requirements are built.** Requirements 3, 5 and 6 shipped with the gate; **1, 2, 4, 7 and 8 shipped with the entry QR code on 15 September 2026**, once the domain existed ([N42](open-questions.md)). Plate and name lookup came first, which requirement 5 already expected to carry most of the traffic. What the code is, in the terms above:

- **A random token issued by the database the moment a booking is confirmed** (requirements 1–2), on every path that confirms one — never the reference or the id. It is **replaced**, not revoked, from the booking screen: the old code stops working at once. **[A]** Replacing answers to `booking.amend`, the permission that already changes the booking (Jeff, 15 September 2026).
- **It opens the booking on the staff host** (requirements 3–4). A guard scanning it with the phone's camera sees this same gate card, with the same moves and the same checks; any other member of staff sees a summary; anyone else sees the first name and an initial, the reference, the dates and whether the booking is confirmed.
- **It travels as an image with the reference beside it** (requirements 7–8): inline and attached in the confirmation email, as a download on the booking screen for the desk to forward over WhatsApp, and **[A]** on the guest's own booking page (Jeff, 15 September 2026) — the address is required now (§13), but an email can be missed, filtered or deleted, and the booking page is the copy a guest can always find again through A9. 480 px at error correction M; printing is not built, so H is never needed.

**[C] The guard is the front desk** ([N54](open-questions.md), Jason, 14 September 2026): he hands the keys over, takes them back, admits day visitors and records the cash he is handed, and he cannot make a booking — where something needs one, the card says *Call the office*. So the screen answers one question per booking, what happens next and who does it, and says why in a sentence. The rules are **[A]**, Jeff's of 13 September 2026, revised for N54:

- **A booking whose deposit is not held in full is not checked in.** That is `check_in_booking()`'s own refusal read ahead of the tap, so the button is never offered only to be refused. The card says which way the deposit is missing — never taken, promised by a transfer nobody has checked, or short — and a guard who may take cash takes it there: the whole deposit, or what is missing off a short one.
- **Money still owed on the stay is said, and does not stop the car.** The ordinary deposit-secured guest pays for the stay on arrival (§9.1), and the card offers to take it; a reader who may not take cash is told the office does. **Nothing is taken while a transfer for the same money waits to be checked**, and a booking the office has not confirmed for any other reason goes to the office.
...
- **A guest due out today, or overdue, is checked out at the gate when the keys come back**, and is listed apart as *Leaving today*. Nobody is checked out at the gate before their last day — the gate cannot tell a guest leaving from one out for the evening, and a check-out cannot be undone — so an early departure is the office's. A guest still owing for the stay is asked for it first: the check-out dialog says that nothing can be recorded against the booking afterwards, and one who leaves owing anyway is the office's to deal with outside the system ([N57](open-questions.md)). Housekeeping keeps its own check-out for a guest who leaves the keys in the unit.
- **Whether the unit is ready is said, never enforced** ([N53](open-questions.md)). The guard hands over the keys with no units board beside him, so an arriving guest's card says when the unit is not marked ready yet.
- **A paid day pass is admitted at the gate, and admitting closes it** ([N40](open-questions.md)). A pass has no unit to leave, so `admit` moves it `confirmed → completed` (§9.2) and nothing has to close it later. Only on its own date, and only paid in full, because nobody meets a day visitor again to collect what is owed — so the guard takes it first. An admitted pass stays on the list for the day, because visitors go out and come back. `check_in_booking()` refuses a pass, so the desk cannot check one in by mistake either.
- **Guests in residence are listed apart**, because their cars come and go all stay.
- **[C] A card whose money is not settled is red** (Jason's team, 19 September 2026; capability D4): a deposit not held in full, the stay or pass still owed, or a transfer nobody has checked — so the guard is careful with it first. It is decided from the booking alone, so every reader sees it, including a phone that is sent no figures, and a *Payment not settled* line says it in words. A closed booking and an overpaid one are never red: neither takes money at the gate.
- **[C] The card shows how many the booking is for, and the guard says when more arrive** (Jason's team, 19 September 2026; capability D7). A stay's card shows its guests and the small ones among them; a pass's, its bands. A small *Extra* button beside the number:
  - **on a stay, tells the office** — an internal note on the booking (§9.7) and an event the office's notifications read, for `booking.amend` holders. The office changes the party (§9.6), and what that leaves owing reaches the card as cash to take. The card says the office has been told until the party changes, so a second guard does not report it again. Reporting never holds up check-in.
  - **on a day pass, lets the guard settle it himself** (**[A]** Jeff, the same day): he names the extra visitors by age band, the pass is priced again for all of them, and he takes the difference in cash under his name, with a note saying so. **Admit is never hidden for it** — whether the guard calls the office before admitting is for the team to agree, not the screen to enforce. Only for a pass on its own day, open, with no transfer waiting, and only for a guard who takes cash; an admitted pass is closed, so there the button tells the office instead. The party change and the cash are the product's existing writers, in that order, so a failure between them leaves the pass owing the difference in red for the ordinary *Take* button, and a repeated press is refused on the headcount it opened on.

**A figure reaches only a phone that may take the money.** What is owed is worked out on the server for a reader holding `payment.record_cash` and for nobody else, so a phone signed in without it carries no prices at all — the rule this block used to state as *never shows a figure*. **The same cash is never recorded twice:** the dialog sends back the figure it showed, and nothing is recorded unless that is still what is owed. The amount rule cannot be that guard, since a stay payment of any size is accepted with a typed reason (§10.5).

**Who did it is the session, not the screen** (requirement 3): the gate calls the desk's own writers under the desk's own permissions — `check_in_booking()` under `booking.check_in`, the check-out transition under `booking.check_out`, `admit_day_pass()` under `day_pass.admit`, and the deposit and payment functions under `payment.record_cash` (§4) — so the booking's history names whoever did it.

**[O] Signal at the guardhouse is still unconfirmed** (register C3). What is built assumes a weak signal rather than none: the list is one small page filtered on the phone, a search is one plain request, and a move that cannot reach the server says that nothing was recorded — except cash, where the money may have been recorded before the answer was lost, so the guard is told to refresh and look before taking any more. There is no offline cache, deliberately — the list carries guests' names and plates, and a shared guardhouse phone is where yesterday's list should not survive. If the answer is *no signal at all*, that is a different build and a data-protection question.
...
**[C] An email address is captured on every booking, and it is required** (Jeff, 17 September 2026, capabilities A1–A4, B2, B17). `guest.email` has existed since the first migration and nothing had ever written it; the public booking form is what architecture.md §9 meant by "email capture is added to the booking form".

**This reverses the 13 September call that it was optional**, on two things that were not true when that call was made. The QR code now exists (capability A8, D3), and it is what the guard scans at the barrier — so a guest who gives no address arrives with nothing to scan, and the fallback that justified optionality is a staff member remembering to forward it. And the desk's own form asked for no address *at all*, so "optional" in practice meant most staff-made bookings had none. It is required on all four ways to make a booking — the website's two forms, the desk and the gate — and `create_walk_in_booking()` refuses a blank one, so the rule holds under any caller. The cost is the one that argued against it: a walk-in who will not give an address cannot be booked on this system. Jeff accepted that knowingly, because the alternative is a guest at the barrier with no code and a guard with no way to admit them.

**[C] Two emails are sent to that address, and never a third** (14 September 2026, capability A8's email half — see [architecture.md §9](architecture.md)). One when the booking is made, carrying the reference, the amounts and the bank accounts; one when the money is verified. The field's own hint on the form says *"For your confirmation. We will not email you anything else"*, which makes this a **standing constraint on every later slice** rather than a preference: a reminder, a receipt per payment or anything marketing would break a promise the customer was shown as they typed. Now that an address is required (above), every booking gets both — which is what makes the promise of *only* two worth keeping.

**Both are delivered from 15 September 2026**, from `noreply@bruneiapartment.com` ([N42](open-questions.md)). The confirmation carries the entry QR code (§12) — inside that one email, so the promise of two emails and never a third still holds.
...
**[A] The number is now *entered* as a country code plus the rest, on all five forms that ask for one** (both public booking forms, the lookup, and the desk's create and amend). A guest ringing from abroad had to guess whether the desk wanted `+60`, `0060` or a bare `12`, and a number recorded without a code is one nobody can dial from outside Brunei. The control defaults to Brunei, so the common case chooses nothing, and it composes what it submits — `+673 8959798`. **It does not touch the invariant above.** A field nobody edits submits the string it was given, byte for byte: every booking taken at the desk before this existed is a bare `8959798` and stays one, which is also what keeps an amendment that only moved a date from recording a phone change in its audit event. No format is validated and no country is inferred — the list is reference data (`lib/domain/countries.ts`), not a rule.
...
- **The unit type's `car_allowance` is shown to the desk, and is a limit for a customer** (§7.1; shown from 17 September 2026). Every form that collects plates against a unit names what the unit includes under the rows. **[C] On the public stay form the rows stop at the allowance** (Jason's team, 19 September 2026): where the *Add another vehicle* button was, a notice sends the customer to the office's WhatsApp to confirm another car first, and the action refuses a booking with more plates than spaces. **[A] The desk's create and amend stay uncapped, with a warning past the allowance**: staff have agreed the extra car with the guest, and §12.5 makes plate lookup the guard's primary path, so a plate the form refused is a car nobody can match at the gate. How many bays the property actually has is still **R3** in the [open-questions register](open-questions.md); it no longer decides whether the customer's form is capped. Day passes are exempt: a pass carries no unit and so no allowance, and visitor parking is the other half of R3.
...
### As built (capabilities A6 and A7) — the guest sends it themselves
...
**[A] A guest's file never touches a staff member's.** `uploaded_by is null` is what separates the two, and it is exact: a staff attach always carries the actor `requirePermission` returned. So a second IC from the guest supersedes the guest's first and leaves alone the front-and-back pair a clerk photographed at the desk; and where a clerk has already filed the slip, the guest's upload is refused and the page tells them it is already on file, which is true. There is deliberately no one-live-identity rule — a booking legitimately holds several ICs — so what is bounded is one guest retaking one photograph, which is the only thing the customer surface made easy.

**[A] One transfer, two rows, two documents.** §10.3 records that a customer settling everything up front sends one sum against a deposit row and a payment row, and that the two stay two. Their one screenshot is therefore evidence for both, and it is filed against each: one upload, one file chosen, one `document` row per money row. Filing it once and cross-referencing would leave one of the two accounting packs assembled without its evidence, and each row carries its own seven-year clock, so they cannot share one. A day pass has no deposit and gets one row; the ordinary online stay pays the deposit alone and gets one row.

**[A] Neither is required, and the IC is still the desk's to collect.** An upload that refused a booking would lose the booking — the reasoning §13 applied to the email address until the QR code made an address the only way in. A guest who sends nothing is a guest the desk registers at the door exactly as it does today. **[O] What is not settled is whether a guest's own upload is *enough* for registration**, or whether the desk should still sight the physical IC on arrival — §13's **[C]** requires a copy and is silent on who takes it. Nothing is blocked: the document is on file either way, and if the answer is that a clerk must still look, that is a line on the check-in screen rather than a change here.
...
The site collected names, phone numbers, car registrations and ICs, and said nowhere what happens to them. **What a privacy notice says is the client's to write and approve, and the product does not decide it** — so what is built is the capability, not the wording. Staff write the policy under *Admin → Website settings → Privacy policy* and publish it to `/privacy`. Until they do, there is no page, and the *Privacy policy* link in the footer of every public page is shown but does nothing. Once one is published, the link works and the guest's IC upload points to it too. Whether a given wording meets the PDPO is **[R5](open-questions.md)**. The following are **[A]**.

**[A] One document, written in a rich text editor.** The policy is one document, not a form of sections, because the likeliest source is a lawyer's document pasted in whole. It is written in a word-processor-style editor that looks like the page a guest reads. The editor offers exactly what the page shows: headings, paragraphs, bullet and numbered lists, and bold. Pasting from Word or Google Docs keeps those and leaves other formatting behind. It replaced a plain text box with typed formatting symbols on 15 September 2026, before anything was published, because a non-technical team should not have to learn symbols (Jeff). What is saved is still plain text, so nothing typed or pasted becomes markup on the site.
...
**One limit is the platform's rather than a policy: 4 MB per file.** WhatsApp screenshots sit well under it and a camera photograph does not, so a photograph — for the public site or for an inspection — is made smaller in the browser before it is sent (§11, architecture.md §8.3). A pack is assembled on the server and never crosses that limit; its own ceiling is 25 MB.
...
All six are live. Day-pass volume against capacity arrived on 16 September 2026, once day passes carried a date and a facility could carry a capacity: the table lists guests per day against `day_pass_headroom()`, the figure the public booking flow enforces. No capacity has been agreed yet ([C2](open-questions.md)), so today it reads *No limit set*. The following are **[A]** assumptions made while building, and are the ones to put in front of the client.

**[A] Day-pass volume is guests, against today's capacity.** A family of five fills five places, so volume is headcount, not passes. It counts every pass whose booking is not cancelled or expired, as the capacity check does. Capacity is the setting as it stands now, applied to every day of the period, because no history of the setting is kept. The period total compares its guests with capacity on every day, including days nobody came.
...
**[A] The deposit line counts arrivals, not rows** (with §11's top-up). A deposit row carries one method and one collection date — how and when it was *first* seen — but what is held can grow afterwards when the desk tops up a deposit that arrived short. Summing the rows therefore lost cash twice: a cash top-up against a deposit that first came by transfer went into the drawer and into no figure here, and a cash top-up against a deposit collected last Monday moved **Monday's** figure, which somebody may already have counted and signed. The line is now assembled from what actually arrived on each day — each deposit's figure less everything added to it since, plus the top-ups taken in cash that day — which the audit trail records exactly, and which is why it needed no new column. A day once counted does not move again.
