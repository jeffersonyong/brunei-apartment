...
- **A guest due out today, or overdue, is checked out at the gate when the keys come back**, and is listed apart as *Leaving today*. Nobody is checked out at the gate before their last day — the gate cannot tell a guest leaving from one out for the evening, and a check-out cannot be undone — so an early departure is the office's. A guest still owing for the stay is asked for it first: the check-out dialog says that nothing can be recorded against the booking afterwards ([N57](open-questions.md)). Housekeeping keeps its own check-out for a guest who leaves the keys in the unit.
