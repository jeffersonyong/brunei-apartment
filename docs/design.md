...
colors:
  ...
  # The subordinate vertical of a date grid, and the one edge in light not
  # drawn in `hairline` (§Components — the booking calendar). The night lines
  # between a tape chart's Monday seams, and nothing else.
  grid-line: "#f2f2f2"

  # The band that says a new unit type starts here on the booking calendar
  # (§Components — the booking calendar). A half-step between the rows
  # (`canvas`) and the header strip above them (`canvas-soft`), so a band
  # scrolling under a pinned header never reads as the header having grown.
  group-band: "#fafafa"
  ...
  # The help desk's gradient stops (§Components — Ask anything): the identity
  # violet and blue at full chroma, deep for light and bright for dark.
  ask-violet: "#7c3aed"
  ask-blue: "#2563eb"
  ask-violet-bright: "#a78bfa"
  ask-blue-bright: "#60a5fa"
  ...
  # Ledger — which book the money lands in: revenue earned, or a liability
  # owed back. Mid hues only, on the same terms as Stream; the mark is a glyph
  # and these reinforce it rather than carry it.
  ledger-stay: "#2563eb"
  ledger-deposit: "#a855f7"
...
themes:
  light:
    ...
    # The help desk's gradient, the one colour on the operations surfaces
    # (§Components — Ask anything).
    ask-from: "{colors.ask-violet}"
    ask-to: "{colors.ask-blue}"
  ...
  dark:
    ...
    ask-from: "{colors.ask-violet-bright}"
    ask-to: "{colors.ask-blue-bright}"
...
- **Ledger colour is a fifth register, and it says *which book the money lands in*.** Two `{colors.ledger-*}` hues on the payment verification queue, where every row is either money for the stay or a promised security deposit, and which one decides what confirming it does to the booking (prd.md §9.1, §11). Built on Stream's terms: **mid only, no tints**, identical in both themes and on every surface, a mark beside its label and never a fill. `LedgerMark` is its one component. The missing tint is again the rule rather than an omission — the queue already spends `warning` on *Repriced* and `positive` on a verified tick, and those two are what mean "look at this" on a row; a tinted chip on **every** row would leave them nothing to stand out against, which is the `available`-is-neutral cost recorded below.
- **The mark is a glyph and the colour reinforces it**, which is the correction worth recording because the first attempt had it the other way round. Two dots at 263° and 293° differ by 30° of hue and **no lightness**, and read as two blue-ish points. **The wheel is spent**: every hue far enough from blue to be plainly different borders one status already owns — rose lands 9° off `{colors.negative}`, lime 18° off `{colors.positive}` — and a red-ish or green-ish point on this screen would say *alert* or *done* about a row that is neither. So shape carries it, exactly as it does for every other non-status register, and the deposit's glyph is the `LockKeyhole` it already wears on the Money card and its own screen rather than a new one. A stay is `BedDouble`.
- **Freed from carrying the load, the hues are chosen for legibility rather than distance.** 41° of hue and 8 points of lightness, both clearing 3.2:1 on the white card and the ink one. The widest available pair does not survive both themes — `#c084fc` is 6.4:1 on ink and 2.64:1 on white, `#1d4ed8` the mirror at 6.7:1 and 2.54:1 — and a mid that must be identical in both cannot vanish on one. Near-misses, in the order they would surface: `stream-short-stay` ≈277° sits 14° off `ledger-stay` and 27° off `ledger-deposit`, `stream-tenancy` ≈323° sits 19° off `ledger-deposit`. None meet today — the queue has no **Type** column, the bookings list no ledger mark — and the glyph is what makes that survivable if one ever does. `{colors.info}` is the system's other blue and never collides, for the reason `stream-short-stay` already rests on: `info` fills a notice panel and is never a mark.
- **The security deposit is not a fifth *tint*, and the request for one is worth recording.** A purple ground under the deposit was proposed (2026-09-05) so staff could tell it from the other gray insets on a booking screen — the identity document, the transfer slip, the accounting pack. Refused, on three grounds that hold generally. A tint that is *always* there is decoration, and a tint has to mean "look at this". Violet is already worn by an identity hue and indigo and fuchsia by two streams, so another purple would be learned against three. And the stage chip already sits on the inset, so a coloured ground would nest one tint inside another and dull the chip that needed reading. Recognition comes from **form**, as it does for every other register: the deposit's mark is the ledger's `LockKeyhole` glyph beside the micro-label *Security deposit* — `Landmark` first, until the cash-up gave banking a Record banking button wearing the same glyph, and a mark shared with an action means neither — standing directly over one three-line figure table — Held, Less charges, the line that is returned or owed — identical wherever a deposit appears (`deposit-figures.tsx`). On the Money card the mark heads the inset; on the deposit screen the section title carries it. The chip keeps the colour, because the chip answers the one question colour is for.

  **A purple *mark* was admitted later (2026-09-19), and the refusal above still stands — it is in fact what the queue ended up applying.** What was refused was a coloured **ground** under a deposit panel, and the reason given was that recognition should come from form: the `LockKeyhole` glyph, identical wherever a deposit appears. The queue now carries that same glyph, which is the paragraph above being followed rather than overturned. What is new is only that the glyph is *tinted* — `ledger-deposit` beside `ledger-stay` — and a tinted glyph is neither of the things refused here: not a ground, and not a tint nested inside a chip. **The one live cost is the third ground** — purple is now worn by an identity hue, by `ledger-deposit`, and indigo and fuchsia by two streams, so a reader learns four. What keeps them apart is form and screen: a face is a circle with two letters, a stream is a bare dot on the bookings register, a ledger is a glyph on the payments queue, and no two appear in the same table. If a screen is ever built carrying a stream dot and a ledger mark on one row, that is the point at which one register has to give.
...
**A card that names itself has a title line, and two things may share it.** The `micro` label sits at the left; **a hint** — the section's explanation, for a section whose behaviour is not obvious from its contents — is a 14px info glyph beside it opening a tooltip, and **the one action that acts on the section as a whole** ("Add note") sits opposite as a `tertiary` button. The hint is a tooltip rather than a paragraph under the content because the explanation is read once and the section is read every day, and a standing paragraph of fine print on a short section made the section mostly fine print; the glyph is focusable so the text is reachable from the keyboard, and it sits *beside* the heading rather than inside it so the heading's accessible name stays the title. **A hint states the rule and stops — two sentences, and never the reasoning behind the rule.** The tooltip is 300px wide and set at caption size, so four sentences is ten lines hanging off a hover, which is a document rather than a hint and gets dismissed rather than read. What a reader needs at the glyph is the thing they would otherwise get wrong — which nights count, what a figure excludes; *why* the rule is that way is a decision, and decisions live in prd.md and architecture.md where they can be argued with. The reporting screens were the case that set this: three hints had grown to 325, 290 and 414 characters, each ending in a clause justifying itself to a reader who had not asked.

**A status column carries a key, and it is not a hint.** A pill is a vocabulary the screen assumes the reader already has: the badge says *Over-banked* and offers no way to learn what that means, how many other states exist, or whether the one on screen is the good one — the label is the whole explanation and it is two words long. `StatusLegend` puts an info glyph on the **column header** listing every state the column can show, drawn as the real badges at the tone and size the rows use, each with one line saying what it means. It lists them all, never just the ones currently on screen, because half the question is what the other outcomes are. A hint explains how a section is *counted* and is prose; a legend is a lookup and is a definition list. A column has at most one, on the header rather than the section title, because it describes that column and not the table. A grid whose colour *is* the column — the booking calendar — has no header to hold one, and puts it on the control line instead. An action on the title line is the record screen's equivalent of the list screen's control-line slot: it is found where a reader looks for it, and the section's body stays the section's content — which is what let the notes composer stop standing open on every booking and become a dialog. At most one of each; a title line with two buttons on it is a toolbar, and belongs on a control line.
...
**A stat strip is one tile per figure, and it takes no container.** The tiles are **cards** sitting directly on the ground (they were gray panels until the ground inverted — see above). A card *around* them was a box drawn around four boxes — the outer hairline bounded nothing the tiles were not already bounding, and it opened the screen with a panel of chrome. What the tiles replaced was worse still: four labels adrift on one wide surface split by dividers, which reads as a spreadsheet header rather than four objects. `card-dark` (ink) is the public site's promotional moment, used at most twice per page. There are no tinted feature cards — colour is not a card treatment. **One status card is the exception, and only one: a gate card whose money is not settled** (Jason's team, 19 September 2026). It takes the negative pair — the `badge-negative` mix for its ground and `destructive` for its hairline — with a `micro` line in `negative-text` saying *Payment not settled*, so the meaning never rests on the tint alone, and its inset turns `card` white, the tone a step away from what it now sits on. At the barrier it is the one fact that must stop the eye before the plate is read; it is a status rather than decoration, which is why it borrows the status pair instead of a hue of its own, and no other card on any surface takes a tint. Interactive cards signal hover by strengthening the hairline and a 1px lift, never a shadow.

**Inputs.** White, hairline, `{rounded.md}` 6px, `body-md` 14px, height 36px (portal 32px), horizontal padding `{spacing.md}`. Focus: the border strengthens to the action colour plus a faint same-hue halo (`ring`) — decisive, not a glow. Labels are `body-sm-strong` ink. Field-screen inputs use the `touch` size. **A password field is hidden until asked** (`PasswordInput`): dots by default, and an eye inside the field's right edge — the search field's clear-button construction, a 16px mute glyph in a 24px target that lifts to ink on hover — shows it and hides it again. It covers every password anyone types or is handed: sign-in, setting your own, and the temporary password an administrator sets or reads back, which Copy still copies without showing.
...
**A rail of named spans** sits beside the grid on a `divider` hairline — Today, Tomorrow, Next 7 days, This month, Next month, Last month, weighted forwards because a bookings list is read to answer "who is coming". A preset is a shortcut to the same value the grid produces, never a separate mode: it is matched by value, so hand-picking exactly this month shows "This month" as selected. Selected is the `canvas-soft` chip, like every other "where am I". **The reporting screens pass a rail of their own, weighted backwards** — Today, Yesterday, Last 7 days, Month to date, Last month, Year to date — because a report answers "what happened", and a forward-looking span on one resolves to a period with nothing in it. Worse, on the occupancy report it would divide by nights nobody could have slept, so "month to date" stops at today rather than running to the month end. The rail is a prop rather than a second picker: the control, its matching-by-value and its chip are one component.
...
**Stay range.** The third of the family, and the last: one day in the form dress is the date field, a span in the filter dress is the chip, and a **span in the form dress** is this. It carries nothing of its own — the chip's two-month grid, the date field's trigger and glyph, the same bookended footer — and it exists because the one span this product asks for *on a form*, a stay, was being asked for as two date fields. Every reason a span is one control is sharper here: two fields can express a check-out before a check-in and a half-filled pair, two clicks cannot, and the number a clerk is actually holding — how many nights — was arithmetic they were doing between them.

**Its second click is the check-out morning**, which is the whole of what separates it from the chip it looks like. `[12, 15)` is three nights: the 15th is drawn as an end, is never a night, and stays sellable to somebody else — the availability calendar's convention and the booking calendar's, not the filter's inclusive pair. It follows that the ends **do not sort**: they are an arrival and a departure rather than two of a kind, so a second click that is not after the first re-anchors, and one on the anchor itself is zero nights and is left half-made. A hover before the arrival paints no band, because a band running backwards says those nights are about to be taken.

**The nights are on the closed control**, after the dates and in mute — `12 – 15 Sept 2026 · 3 nights`. Without them a stay and a filter range render the same string for two different spans, which is precisely the thing a desk gets wrong, and the count is what a clerk is reading back to the guest anyway. The footer says it again with the departure named outright, and asks for the *check-out morning* mid-pick rather than the "last day" the filter asks for. There is **no Clear**: a booking with no dates is not a state its form has any use for.
...
**The public availability calendar (A1).** A different component from the range picker above and from the booking calendar below, and the only calendar in the product that carries **information in the cell**. A staff member filtering a list already knows the dates they want; a customer is choosing them, and the two things they choose on are what is free and what it costs. Two months side by side from `md` up, one below it with both arrows on it, on the same Monday-first six-row grid as everything else.

**A cell is a night**, and it holds the numeral with that unit type’s rate beneath it at 10px `tabular-nums` mute. A night nobody can sell says **Full** in `micro` where the rate would be. No strike-through and no red: a sold-out Saturday is a fact rather than a warning, and colouring it would make a calendar of ordinary weekends read as a page of errors — the same reason `available` is a neutral badge on the units board. A full night is drawn and is **not a tab stop**; tabbing through a fortnight of sold-out Saturdays to reach the free week is not navigation.

**Today is a weight, not a dot.** Every other calendar marks it with a dot under the numeral, and here that row is spoken for by the price — a third line in a 48px cell. The numeral goes 500 instead. It is the one place these calendars deliberately differ, because this cell has a job the others do not.

**The second click is the check-out morning**, which is where this parts company with the range picker’s inclusive ends. A stay `[12, 15)` is three nights, the 15th belongs to nobody, so the 15th is drawn as an end, is not charged, and **does not have to be free**. Only the first click has to land on a night with something left. Everything else is the range picker’s language unchanged: the band between the ends is `canvas-soft`, the ends take the action fill — lagoon here, which is the one place this grid takes colour — and a far end still being chosen is the drawn white chip.

**The footer is the running answer**, on the grid’s own hairline: *Pick the day you arrive, then the day you leave* before anything is chosen, *Now pick the day you leave* mid-selection, then the nights and the range once both ends are in. It carries `aria-live`, because the number of nights is the fact a customer is checking and it changes without the page moving.

**A unit type the building has no units of is not offered at all.** Changing the type repaints the grid and keeps the range.

**Public booking forms.** One card, sections divided by hairline rules under `micro` headers — set as a `<legend>` in the bordered `<fieldset>`, so the rule resumes where the label ends, with padding on the **trailing edge only**: padding on the leading edge would shift the label off the left edge its own fields align to — — *Which unit* · *Your dates* · *Who is coming* · *Extras* · *Your details* — at the customer surface’s 36px control height. The itemised quote sits beside it in a card that sticks from `lg` up and stacks beneath on a phone, carrying the screen’s one primary fill at its foot: the button belongs with the figure it commits to. The security deposit is a `Notice` under the total rather than a line in it, because prd.md §11 makes it a refundable liability and not part of the price. A count field is a number input at ~110px, never a stepper — a family of nine types 9 faster than they tap it — and **its column is a fixed width while its input is not**: left to itself a flex item is as wide as its widest child, which is the hint, so a long one ("BND 28.00 each, with a pillow and blanket") made its field three times the width of its neighbour and the gaps between fields read as arbitrary rather than as a rhythm. The column is fixed so the row is even and the hint wraps inside it. The honeypot is `hidden` **and** `aria-hidden` with `tabIndex={-1}`: a field hidden from sight but not from a screen reader is a trap for the people least able to recover from it.

**Transfer instructions.** The QR block’s construction without the QR: the reference in `font-mono`, the amount at `display-sm` `tabular-nums`, the bank accounts as a `<dl>` with their numbers in mono, one `Notice` carrying the reference to quote, and one primary. **No countdown** — [open-questions.md](open-questions.md) N7 makes the hold indefinite, and a timer the system does not enforce is a promise it does not keep.

**Booking lookup.** Two fields and a button, and the one public form with **no `<legend>`**: the form idiom above divides sections with hairlines under `micro` headers, and a form with one section has nothing to divide — a legend naming the only group on the screen would label a rule that is not drawn. The reference field's *value* takes `font-mono` while its label stays Geist, because what is typed there is a reference. The refusal is a `Callout` above the card and **never a `FieldError` beside one field**: naming which half was wrong would confirm that the booking exists, which is the one thing the screen withholds, so four different failures render one sentence.

**The FAQ.** A topic heading stands on the page ground at `display-xs` over one card; the card holds its questions as **disclosures** ruled apart by hairlines (`FaqDisclosures`). Each is a native `<details>`: the `<summary>` carries the question as an `h3` in `body-md-strong` ink beside a 16px mute chevron that turns over when it opens and lifts to ink on hover, so the row reads as something to open without becoming a button; the answer is `body-md` copy beneath it at reading width. That is the two-step ladder rather than `micro` — a question is content, not a label on content, and a sentence set at 11px uppercase is unreadable. Topics separate on hairlines, no tinted band is spent, and one dark closing moment out of the two. **It printed every answer open until 14 September 2026, and native `<details>` is what let it stop without losing the reasons it had:** a browser searches inside a closed disclosure and opens it on a match, so Ctrl-F still finds an answer; the answer is still in the markup a search engine indexes; and it opens with no script at all. What it adds is a page a guest can scan by question. Several can be open at once — comparing two answers is a reasonable thing to want — and each row keeps the stable `id` staff link to, which opens its answer when a page loads at that address. The landing page shows the few staff chose in the same construction, without the ids, beside a `tertiary` *See all FAQs*: that band sells nothing, and the page's lagoon fill belongs to booking.

**The privacy policy.** A page of prose at the FAQ's reading width (720px): a lagoon `micro` eyebrow, the Fraunces title, and "Last updated" in mute. Then a hairline, and the document. Staff write every word of the document, and it renders in one component (`PrivacyPolicyDocument`): section headings at `display-xs` ink, paragraphs and lists in `body-md` copy at 64ch, bullet and number marks in mute, and bold as `font-semibold` ink. **No card, no disclosures, no closing band.** A notice is read top to bottom and nothing on it is for sale, so it borrows none of the FAQ's scanning devices and spends none of the page's two dark moments. **The portal edits it in place, in the same tokens.** The rich text editor (`PolicyRichText`) draws the document with that component's type and spacing, so there is no separate preview.
- **Frame:** the Textarea's frame (hairline, 6px, focus ring).
- **Toolbar:** a hairline-ruled strip across its top. Its 32px ghost icon buttons are Heading, Bold, Bullet list, Numbered list, then a divider, then Undo and Redo. Each button carries its name as a tooltip and an accessible label. A formatting toggle that is on where the cursor sits takes the `muted` chip and ink, the tab chip's construction without the lift.
- **Register:** the editor carries no Fraunces and no lagoon, so it sits on the monochrome operations register as it is.

**A link to a page that does not exist yet is shown inert, not hidden** (the footer's *Privacy policy*, until staff publish one). It keeps its place in the list at 50% opacity with a default cursor. It is a `role="link"` with `aria-disabled` and no `href`, so a screen reader announces it as a link that does nothing yet. Inline links that would sit mid-sentence (the IC upload's) are left out entirely until the page exists, because a dead phrase inside a sentence reads as a broken promise rather than as something coming.

**The closing band.** One construction (`ClosingBand`), used by the landing page and the FAQ: ink ground, `display-md`/`display-lg` headline, copy at 75%, and a `variant="inverted"` primary beside at most one outline action drawn with a hairline of the inverted foreground — a `ghost` button is invisible on ink. Its content column matches the page above it, wide on the landing grid and reading-width on a page of prose, so the band's contents line up with the column rather than stepping out of it.

**Public header.** Two actions at every width: a `tertiary` *Find booking*, and **Book** — the header's one lagoon fill — opening a menu onto the two things a customer can book online, *Day pass* and *Stay* (14 September 2026; it was *Book a stay*, which sent a family after a day pass to an apartment calendar, and both buttons were hidden below 640px, which left a phone with no way to book from the header). Each menu item is its title in `body-sm-strong` ink over a `caption` line in mute saying what it is, because *day pass* is this business's word for something a first-time visitor may not know it sells; the lines name no facility and no figure, since both are settings. Below 360px *Find booking* folds to its search glyph and keeps its accessible name, so the pair never pushes the brand off the row. Below 640px the day-pass, stays and long-term text links stay hidden, and the theme toggle leaves the header for the footer rather than crowd the two actions.
...
**A cash-up state is an outcome, and takes the semantic set for it.** The daily cash-up's five states are not stages of a workflow — a day is settled or it is a question for somebody — so *balanced* is **positive**, *not banked* **warning** (outstanding rather than wrong, the reading `awaiting_inspection` has on a deposit), *no cash* **neutral**, and **short and over share negative deliberately**. Neither is worse than the other: less money reaching the bank than the desk took, and more, are the same class of problem, and splitting them by colour would imply one is recoverable and the other is not. The variance figure itself stays ink — the badge beside it carries the colour, and a column of figures should read as a column.
...
**A unit's status badges** (added 2026-09-02, with the units board). A unit's state is mostly its occupancy's state, so most of these are the booking tones read through from the other side of the same fact: `held` keeps `warning`, `booked` keeps `positive`, and `occupied` keeps the brand pair `checked_in` carries, because it *is* checked-in seen from the unit. Two are new: `out_of_service` takes `negative` — a unit nobody can be put in is the one state on that board that costs money until somebody fixes it — and `leased_long_term` takes `neutral`, because a lease is a settled arrangement rather than something to act on. **The turnover's two arrived with the housekeeping phone screen** (2026-09-25): `awaiting_inspection` takes `warning`, the reading the same words already have on a deposit — somebody owes this unit a visit — and `cleaning` takes the in-progress pair `active`, because the visit has happened and the work is under way. The board and the cleaner's phone draw them from the same table.
...
**Matrix tables.** A table whose *rows* are the subjects and whose *columns* are attributes — the roles screen's twenty-three permissions across five roles. It is the ordinary table chrome with four additions, and it is the shape to reach for whenever a screen's real question is comparative ("who can verify a payment?"), because a one-at-a-time switcher answers that only by being visited once per column and remembered. **The identifying column is a `<th scope="row">`** (`TableRowHead`) at cell metrics, not header metrics — it sits in the body. **Group headings are a row of their own**: the group's name in `micro` mute on the card fill with the divider above it and none below (`divide-y` rules each row along its bottom, so the label's own bottom rule is suppressed or it reads as a boxed heading). That is `FormSection`'s grammar, deliberately not a second `canvas-soft` strip — the header is the only gray. **The booking calendar is the exception**, and only because of its width: a label and a gap say "a new block starts here" across eight columns, but across thirty night columns the gap reads as an empty row and the label as a stray, and a reader scrolls past a type boundary without seeing it. There the heading is a `group-band` strip running the full width of the grid — one object the eye can follow out to the last day of the month. **Quiet, though, and carried by three cues rather than one.** It was a tone *below* both its neighbours, 32px tall and ruled top and bottom, which made a divider the heaviest thing on the screen — and its bottom rule stacked with the first unit row's own top border, so in the separate model that seam drew 2px. It is now a half-step *between* the rows and the header, 28px, with **one** hairline above and none below, and the type's name in **ink** with only its count in mute. The ink label is what tells the band from the header now, which is what the heavier fill was buying; a strip thirty columns wide needs very little contrast to read as an object. **Layout is fixed, not auto**: auto layout hands the slack to the widest column and strands every control a hand's width from the label it belongs to, so the columns are declared and the **container takes its width from the table** (`w-fit`) rather than capping at the same number — a `max-w` is a border-box, so capping at the table's width leaves a content box two hairlines narrower than the table and puts a scrollbar under a matrix that fits. `max-w-full` is what makes it scroll when the panel really is too narrow, and there **the identifying column pins** (`Table scrollX`, `sticky left-0`); a pinned cell needs an opaque fill and its own `group-hover` tint, since the row's hover would otherwise pass behind it. **A screen's action aligns with the right edge of the surface it acts on, not the panel's.** On a full-width table the two coincide, which is why the Staff tab's "+ New staff account" never looked like a decision; on a constrained surface they do not, and a Save pinned to the panel edge reads as floating loose of the table it writes. The whole cluster — status line, action, any callout, the table — takes one declared width, so they share an edge by construction rather than by two numbers being kept in step. **Hover is the row alone.** A column highlight was tried and dropped: the row rule plus the pinned name already carry the eye across, and a second axis lighting up on every pointer move is movement the screen does not need.

**The booking calendar (B1).** The matrix idiom over a date axis: rows are units under `micro` group rows per type, columns are nights, and a bar is a cell spanning the nights it holds — a real `<table>`, so the structure a screen reader gets is the structure a sighted reader sees, and a colspan is the exact primitive for a thing that can never overlap its neighbour (G1). **Nights are half-open**: a stay `[14, 16)` paints the 14th and the 15th and the 16th stays free, matching the exclusion constraint, and a bar that reached the check-out column would draw a collision the database permits. **What is drawn is what the constraint blocks** — every booking not expired or cancelled, every lease, every out-of-service period — which is wider than the units board colours: a completed stay whose last night has not passed is *awaiting inspection*, *cleaning* or — once marked ready — *available* there, and a bar here until that night passes, and the calendar is the honest one about what can be sold. Day passes occupy no unit and are not on it; the page's description says so. **A bar is the badge's construction at row scale** — the same tone from the same status table, the 6px radius, `caption` text — with a 2px inset at an end inside the month so back-to-back stays meet with a seam, and a square end with a chevron where the month cut it, so a lease with no end visibly runs off the grid rather than seeming to stop on the 30th. A bar is a link to the record, never a drawer. **Colour is spent on bars only**: a free night is uncoloured (`available` is neutral), today is a chip around its numeral and never a column fill, and hover is the **night**, not the row — no weekend tint. Today wore the picker's dot until 2026-09-09, and it does not survive the move: the picker gives it a 36px cell to sit in, the calendar's header gives a three-line stack about 4px, so the mark sat all but on the header's rule and read as a smudge. It is now the "where am I" chip the pagination footer and the nav already use — a card fill with a hairline at the control radius — in the flow, so nothing has to be held clear of anything, and every numeral in the row sits in the same 20px box so the chip cannot shift its own column's baseline. **Hover is the exception to the matrix rule**, and for the reason the rule exists: on the roles matrix the row tint carries the eye across to a value, but here the *cell* is the thing being pointed at — a click on it starts a booking, and it answers with a `+` on the `muted` fill. Tinting the other twenty-nine cells at the same moment left the one cell that meant something a shade apart from thirty that meant nothing. The rules are on every row here and the unit column is pinned, which is what the tint was for. **The grid is ruled on both axes**: row dividers, a `divider` seam on each Monday, and a `grid-line` vertical between the days within a week. That last one is the single exception to light drawing every edge in `hairline`, and it is deliberate — the columns of a tape chart are an *axis*, not a table's rows, and reading a bar off one means counting nights across 32px of nothing. A subordinate line at a regular seven-column rhythm reads as an axis rather than as two grays disagreeing; it is used here and nowhere else. **Both axes scroll inside the grid** (`Table scrollX scrollY`), the one table on the surface that is its own vertical scroller: forty-eight rows outrun a screen, and a row thirty columns wide is unreadable once its date headings have scrolled away, so the header row and the unit column both pin. That forces `border-separate`, since collapsed borders do not travel with a sticky cell; every rule is a cell's border. It also makes the grid's height cap load-bearing: the panel is the surface's own scroller, so a grid one pixel too tall gives the screen **two** vertical scrollbars. The cap names what is actually above and below it — the chrome, and the panel's own `3xl` foot padding — rather than standing in for both with one number, and keeps a little slack, because the chrome above is content that can wrap. **A stay is chosen with two clicks on the free nights of one row** — the first is the arrival, the second the departure, both of them *dates* as a guest says them. The nights between fill with a band at the bar's own geometry, so what is on screen is the shape of the stay about to exist; the departure column is never in it, because `[14, 16)` is two nights and the 16th stays sellable. A second click that is not after the first re-anchors rather than swapping the ends — the range picker takes its two ends in either order because both are the same kind of thing, and here one is an arrival and one is a departure. A span may not cross a night already held, or one the screen would not sell. The band is ink at a tenth, not `muted`: `muted` is `canvas-soft`, which is what the row already fills with on hover. **The two clicks create nothing** — they open a dialog naming the unit and the dates, and Continue hands the booking form the unit, the type and the range. A night is *not* a tab stop — fifteen hundred of them would make the grid impassable by keyboard, and the control line's primary is the same action in one; bars stay real tab stops in reading order. The `+` a free night shows under the pointer is a pseudo-element rather than an icon (`night-plus`), because there is one per cell and up to fifteen hundred cells. The month is the view, not a filter: the current month is the absence of the param, the arrows step it, and Clear drops the type narrowing but keeps the month. **Only the units something happens in are drawn**, with "Show empty units" to widen it — forty-eight rows of which six carry a bar is a grid that has to be scanned rather than read, and the question it is opened with is almost always about the six. The count says "6 of 48 units" whenever rows are being held back, so what is on screen is never mistaken for the size of the building. The control line reads the way every list screen's does — **what narrows the grid on the left, what acts on it on the right** — and the month belongs to the right. Stepping it is not narrowing a list, it is moving the view, so it stands beside the primary rather than among the chips; the type filter and "Show empty units" keep the left, where a reader already looks for them. Between the two the grid reports on itself, in the count and the legend. The two clusters navigate independently, each with its own transition, so a month arrow does not dim the type chips it did not change. **There is no "Today"** — the arrows are how the month moves, the screen opens on this month unasked, and a shortcut back to where you started earns its place on a picker a customer meets once rather than on a grid the desk lives in. Its legend sits on the control line, because the colour *is* the column.

**A table's own export.** Every list screen ends its control line with a `tertiary` **Download CSV** — the reports screens' button (E5), generalised. It is an anchor wearing a button, always: the response is a file with a name on it and has to survive a middle-click, a right-click and a browser that has not run the page's JavaScript. **Where it goes** follows the reports rule already: the actions end of the control row where a screen has one, the top-right of the title line where it does not, the end of the tab row on a screen that is all tabs, level with the tabs and directly above what it exports (Property settings, and Roles & staff, where it sits before New staff account). It sits *before* the screen's primary, so the filled button stays last. **A screen with satellite tables opens a menu instead of downloading at once** — the register hands over its bookings, priced lines, vehicles, notes, guests and documents as separate sheets, because a column of nested JSON is not something an accountant can use — and a screen with one table is a plain button with nothing to open. Both wear the same label, because they are the same offer. **A view export and a table export are never mixed silently**: on the cash-up, where both exist, the menu names them apart ("Day by day (as filtered)" against "Cash banked"), since one is the screen with its period and filters on it and the other is the whole table. There is **no Export data screen**: a list of seventeen table names filed under Admin is a schema browser, and whoever wants the bookings as a spreadsheet is already looking at the bookings. What that screen was carrying — the guarantee that *every* table has a way out (F5) — is carried by a test instead.

**A column nobody controls is the one that must not size the table.** The audit log's *Reason* is free text somebody typed: most are four words and one in twenty is a paragraph, and under auto layout that one row set the width of *When*, *Who* and *What* for every other row on the screen, so the table's proportions changed from page to page. So the columns are declared — **and the elastic one is not the free-text column but the one the screen writes itself**. Under fixed layout the surplus goes entirely to whichever column is left undeclared, so that column ends up the widest on screen; *Reason* is the worst candidate for it twice over, since it is empty on most rows and was taking three hundred pixels to say "—". *What* takes it instead: a closed vocabulary of short sentences that wraps to a second line without complaint. Reason is fixed at a width that shows the opening of a sentence and **clips to one line with a quiet "Show more" under it**, unfolding the row in place. That also makes the offer exact rather than approximate: the column is the same width on every screen, so a character count answers "is this clipped?" — which is worth a great deal more than a ref and a resize observer on a table that is otherwise entirely server-rendered. The control goes *under* the sentence rather than beside it, where it would eat a third of the width from the text it describes, and it costs nothing there because the Record column already stacks a reference over its kind: no row on this table was ever one line tall. **Cells stay centred in their row**, as they are on every other table on the surface — top-aligning them to serve a state a reader has to ask for made the ordinary row read differently from the register beside it.
...
**QR block.** White card, centred QR ≥ 200px with default quiet zone, booking reference in `display-sm` beneath, guidance in `body-sm`. **Three places, one construction** (entry QR code, 15 September 2026):

- **The guest's booking page** — the block as written, the code at 240 px, and *Save the image* as a `tertiary` anchor under the guidance. The page's one teal fill stays on the action it already has; a code is something to keep, not something to do.
- **The booking screen** — an *Entry QR code* section card: the code at 200 px on a hairline-bounded white square, beside the reference in `display-sm` mono and one guidance line, then *Download image* as a `tertiary` anchor. *Replace code* is `tertiary` on the title line, like the notes' *Add note*. The operations register, so no teal.
- **The confirmation email** — the block at 240 px under the intro, taking the reference line's place so the reference is set once, in mono, beneath the code.

**The code is black on white in every theme.** A scanner reads contrast, not tokens; a code drawn in `foreground` would invert in dark mode, and not every scanner reads an inverted code.
...
**Search.** The bar opens a dialog anchored high on the page, not centred, because its height changes as results arrive. There is no dialog header: a 48px row holds a 16px mute glyph and a borderless `body-md` input, over a `divider` rule. Results are grouped under `micro` labels (Screens, Bookings, Payments to verify, Deposits, Units). Each row has the record's reference in Geist Mono on the left, then a `body-sm-strong` title over a `caption` mute detail line. The active row is a `muted` chip, inset from the dialog's edges and at the 6px control radius. The arrow keys move it, Enter opens it and the pointer follows it, so the keyboard and the mouse never point at two rows at once. Before anything is typed, the empty panel says what the box can find. The shortcut chip reads ⌘K on Apple devices and Ctrl K elsewhere.

**Ask anything** (the staff help desk; Jeff, 18 September 2026). A link in the portal header, just left of the search field and split from it by a 20px vertical `divider` hairline, centred in the gap — two ways of finding something, side by side but read as separate tools — that opens the ChatGPT help desk project in a new tab. A 16px `Sparkle` glyph and the `body-sm`/500 label **Ask anything**, borderless, so the bordered search field stays the one object in the header; below `md` it folds to the glyph, as the search field folds to its icon. **It is the one coloured thing on the operations surfaces:** label and glyph always carry a static `ask-gradient`, violet into blue — the identity set's two hues at full chroma, deep stops on the light panel and bright ones on the dark, because the identity pair itself read as flat indigo in light and near-white in dark. Hover and focus add only the `muted` chip, as on every other ghost control; the colour is the invitation, so it does not wait to be found. The weight stays at 500 — checked against 400, which read thinner than the plain label beside it. This breaks the monochrome rule on purpose, and the break is held to this control: it is an invitation to something outside the product, the AI register is what makes it recognisable as that, and colour spent anywhere else would start to read as meaning. No shimmer — the rule against sweeps stands.

**Notifications.** The bell is a `{spacing.control}` icon button. When something is new it carries an 8px **ink** dot at its top right, ringed in the panel tone so it stands off the glyph. The dot is not a status colour: "something new" is not a state of the business, and the surface is monochrome. There is no count on the bell, because nothing is a pill; the number is in the panel's header ("3 new"), and the button's accessible name carries it too. The panel is a `menu`-scale popover, 360px wide, aligned to the bell's end. It has a `body-sm-strong` title over a `divider` rule, then rows split by hairlines. Each row has a 16px mute glyph for its kind, a `body-sm-strong` ink title over a `caption` mute detail line (reference · guest · amount), and on the right the time since in `caption` tabular figures, with a 6px ink dot under it while the row is new. A row that opens something lifts to `muted` on hover and focus. An empty panel says what would appear there.
...
**Photo slots** (capability F7). A photograph on the public site sits in the aspect wrapper its placeholder already reserved — 4:3 for the hero and the day-pass and stays cards, 1:1 for the "Follow along" tiles — so adding or removing one never moves the layout. It fills the box (`object-cover`) at the framing staff chose, as its `object-position`; it keeps the wrapper's `{rounded.md}` hairline and `muted` fill, which is what shows while it loads; and **it carries no text over it** — headings and copy stay on the card beneath, where the two-step ladder still reads. Only the hero loads eagerly. **In the portal a photograph is content, not chrome**: it is the one full-colour thing on the monochrome operations surface, and every preview is drawn at the site's own shape and framing, so staff see the crop a visitor will. An empty place there is the recessed empty-state panel with the one action that fills it. **A public file field says the opposite sentence to a private one**: where an IC upload says the file is stored privately and deleted on a schedule, a photo upload says the photo will be public on the website and asks for none in which a guest can be recognised without their permission. The *Keep in view* picker is nine native radios in a fieldset — so the arrow keys and a screen reader work without anything being built — drawn as control-radius squares, with a status dot marking the choice.
...
**Each screen has a skeleton of its own.** Its fixed title (and, on the public site, its eyebrow and Fraunces headline) is printed as itself, because that is what tells a reader the click landed. Anything the server reads is a placeholder the size of the real thing, so the arriving screen replaces it without moving the page. A table skeleton draws its header strip without labels. A title that is the record's own name (a booking reference, a unit) is a bar.

**Error screens.** A screen that failed to load keeps its surface's chrome and shows the empty state's construction (a `muted` panel, centred): "This page didn't load", one sentence on what to do, **Try again** as the region's primary, a `tertiary` way home, and the server's reference in Geist Mono for whoever follows it up. It is not a negative callout. A callout reports a refusal the reader caused, and the reader caused none of this. It never shows the error's own message, which is an engineer's sentence at best. Each surface names its own help: the office on the operations surfaces, WhatsApp on the public site.

**Text actions.** An action worded as a sentence rather than built as a button: `body-sm` in the copy tone, underlining on hover, with the standard focus ring. The "Forgot password" register. It is for the places a control is a real action but the screen has as many bordered rectangles as it can carry — a section's `micro` title line, a field's label row — where one more reads as chrome the section has grown rather than as an offer. **Underlined at rest, and the underline is what changes on hover** — `muted-foreground` at rest, `foreground` under the pointer, at `underline-offset-2`. Hover-only was tried and is wrong: ink text with no fill, no border and no line is body copy, and a control nobody recognises until they point at it is not discoverable. **One exception: the sign-in screen's *Forgot password?*** underlines on hover only. It sits on the Password label's row, where every sign-in screen puts it, so its place says what it is and a line at rest only adds weight to a card that is otherwise one form. **Tone, not thickness:** a line that thickens reads as jitter, because the word appears to move; a line that darkens is the step the rest of the system already makes. The *text* stays ink throughout, and that is why the step is on the line — `copy` and `foreground` are the same ink, so muting the label itself would make the action read as something naming content rather than offering it. No hue is involved: on monochrome surfaces the whole affordance is one tonal step on a 1px line, a pointer cursor and the focus ring. Not a `Button` variant: the button owns a height and a padded box, and a link that unsets both is a variant fighting its component. Monochrome on every surface. **A text action that exists but is not switched on yet stays where it will be, inert** — the same words in `muted-foreground` with no underline, announced as a disabled link and focusable so a tooltip can say why, with a sentence nearby saying what to do instead, because a tooltip never opens on a phone. The sign-in screen's *Forgot password?* is the case: shown from the start so staff learn where it is, and live the day email can be sent.
