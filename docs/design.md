...
**Ask anything** (the staff help desk; Jeff, 18 September 2026). A link in the portal header, just left of the search field — the two ways of finding something side by side — that opens the ChatGPT help desk project in a new tab. At rest it is a ghost control like the bell: a 16px `Sparkle` glyph and the `body-sm`/500 label **Ask anything** in `muted-foreground`, no border, so the bordered search field stays the one object in the header; below `md` it folds to the glyph, as the search field folds to its icon. **On hover or keyboard focus it is the one coloured thing on the operations surfaces:** the label and glyph take a static `ask-gradient`, violet into blue — the identity set's two hues at full chroma, deep stops on the light panel and bright ones on the dark, because the identity pair itself read as flat indigo in light and near-white in dark. The weight stays at 500: the colour is the change, not the type. **The gradient exists only while hovered or focused** — text clipped to a background is drawn with different smoothing and reads visibly heavier, so a label carrying the gradient invisibly at rest looked bolder than the controls beside it. This breaks the monochrome rule on purpose, and the break is held to this control: it is an invitation to something outside the product, the AI register is what makes it recognisable as that, and colour spent anywhere else would start to read as meaning. No shimmer — the rule against sweeps stands.
