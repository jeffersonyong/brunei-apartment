...
**The privacy policy.** A page of prose at the FAQ's reading width (720px): a lagoon `micro` eyebrow, the Fraunces title, and "Last updated" in mute. Then a hairline, and the document. Staff write every word of the document, and it renders in one component (`PrivacyPolicyDocument`): section headings at `display-xs` ink, paragraphs and bullet lists in `body-md` copy at 64ch, bullet marks in mute. **No card, no disclosures, no closing band.** A notice is read top to bottom and nothing on it is for sale, so it borrows none of the FAQ's scanning devices and spends none of the page's two dark moments. **The portal's preview is that same component**, headings dropped to `h3` inside an inset panel. It carries no Fraunces and no lagoon, so it sits on the monochrome register as it is.
