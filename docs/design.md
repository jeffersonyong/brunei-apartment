...
**The FAQ.** A topic heading stands on the page ground at `display-xs` over one card; the card holds a `<dl>` ruled between entries; the question is `body-md-strong` ink and the answer `body-md` copy. That is the two-step ladder rather than `micro` — a question is content, not a label on content, and a sentence set at 11px uppercase is unreadable. Topics separate on hairlines, no tinted band is spent, and one dark closing moment out of the two. **No accordion, and no disclosure component is added for one:** every answer stays in the DOM, so a reader's Ctrl-F finds it, a search engine indexes it, and staff can send a link to a single answer — which is what the stable `id` on each entry is for.
