...
> - **A customer books as many cars as the unit parks, and messages you for more.** The stay forms name the unit type's parking under the registration rows. **On the public site the rows stop there** (your team's request, 19 September 2026): where the *Add another vehicle* button was, a notice says to message you on WhatsApp first to confirm another car, and a booking with more plates than spaces is refused. **Your own forms still record every car**, with a warning past the allowance — your staff have agreed the extra car already, and your guard looks arrivals up by plate, so a registration the form turned away is a car he cannot match at the barrier. The allowance is the one you set per unit type; how many bays the building actually has is still a question for you (R3). Day passes are unaffected.
...
>
> Six things worth saying plainly:
>
...
> - **A confirmed stay is told how to check in, and every confirmed guest where to find us** (added 19 September 2026). The confirmation email and the guest's booking page carry the property's five check-in steps — Security Counter, registration form, the booker's IC, the key and a parking space, the Wi-Fi sticker — and both carry the address with a link to the property's Google Maps pin. A day pass gets the address but not the steps, which are about an apartment. While a stay is still being checked, its page says the instructions follow once it is confirmed. The public site shows the address and an aerial map in a *Getting here* section on the landing page and in the footer of every page.
...
| B18 | Change how many people a booking is for after it was sold — a short stay, **including one already checked in**, or a day pass. The price is worked out again for the new party and the difference is left on the booking: more people and the guest owes it (and the gate shows it as cash to take); fewer and the booking shows what is owed back to the guest, which you settle with them directly. On a stay only the extra-person charge moves, for every night of the booking; the nights and extras keep the prices they were sold at. **(added to the original scope at your team's request)** |
...
| D4 | See the booking's payment status at the gate, so an unpaid arrival is flagged rather than waved through — and paid there where the guard can take the money (D6), or routed to the office where he cannot (N54). **A booking whose money is not settled — deposit not in, stay or pass still owed, or a transfer nobody has checked — shows as a red card**, so it stands out before anything else is read (your team's request, 19 September 2026). |
...
| D7 | See at the gate how many people each booking is for, and say so when more arrive: on a stay the guard tells the office in two taps, which leaves a note on the booking and a line in the office's notifications; on a day pass he adds the extra visitors himself and takes the difference in cash on the spot. **(added to the original scope at your team's request)** |
...
| F14 | Tell confirmed guests about food — there is no restaurant at Palm Villa, so the booking page and the confirmation email say how to order from the outside provider, with their number to call and a link to their menu flyer. Change the words, the number or the flyer from the portal when the provider changes, or empty the words to stop saying anything — **added 19 September 2026** |

> **Website settings.** F7, F9, F10 and F14 are the four tabs of one screen, *Admin → Website settings*: Photos, FAQs, Privacy policy and Food. Each tab has its own permission, so a member of staff sees only the tabs they have been given — except Food, which comes with Photos, because the notice is the menu flyer's caption.
