...
| A8 | Receive booking confirmation and an entry QR code by email — delivered as a forwardable image so staff can also send it in an existing WhatsApp conversation — **delivered: the emails 14 September 2026, the entry QR code 15 September 2026; see the notes below and under D** |
...
>
> **The entry QR code (A8's other half) followed on 15 September 2026** — see the A8 note below.
...
>
> - **A9 mattered more than its one line suggests while the email was off.** Until the sending domain was agreed (register N42, 15 September 2026), a customer who closed the tab had *no* route back to their booking. It is still the only route for a guest who booked without an email address, or who has lost the email.
...
>
> - **The QR code followed a day later, on 15 September 2026.** It waited for the domain, because a code issued before the domain existed would have stopped working the day it changed. A confirmed booking now gets its code the moment it is confirmed. The confirmation email shows it and attaches it as an image a guest can forward to whoever is driving; the guest's own booking page shows it too, for a guest who booked without an email address; and the booking screen has a *Download image* button for the desk to send it over WhatsApp. A guard scans it with the phone's normal camera and lands on that guest's gate card (D3). Anybody else who scans it sees only a first name and initial, the reference, the dates and whether the booking is confirmed. If a code is forwarded somewhere it should not be, the desk replaces it from the booking screen and the old one stops working at once.
> - **The emails are on, from `noreply@bruneiapartment.com`** (register N42, answered 15 September 2026). Until the domain was verified the mail service would only deliver to the developer's own address, so the feature was finished and switched off; with `bruneiapartment.com` verified, both emails and the staff *Forgot password?* link now reach real inboxes.
...
> - **The QR code arrived on 15 September 2026, and plate lookup stays first-class.** A guard scans a guest's code with the phone's normal camera and lands on that guest's card, with the same buttons and the same checks as on the list — scanning saves typing a plate and changes nothing about what he may do. If his sign-in has lapsed, he signs in from that page and lands back on the guest. A code lets nobody in by itself: anyone else who scans it sees a first name and initial, the dates and whether the booking is confirmed. Plate and name lookup — which the requirements always expected to carry most of the traffic — is unchanged.
>
...
> **F9 as built.** The FAQs are managed from *Admin → Website settings → FAQs*, a tab beside the photos. The screen opens on what is on the front page (up to six, in page order), then every topic with its questions — each showing its answer as the site will show it, its address (`/faq#…`) and who last changed it — and a question is put on the front page, moved up or down, edited or removed from its own row.
>
...
>
> - **Three kinds, on purpose.** A website booking (to anyone who can see bookings), a payment to verify (to anyone who can verify payments) and a booking email that failed (to anyone who can see bookings). Holds do not expire (N7), so there is no "hold expired" notification.
