 ...
 *
 * `/deposits/PV-4821`, not a uuid. A deposit has no reference of its
  ...
  return (
    <div className="max-w-[1120px]">
        ...
        actions={
          <>
            <Button asChild variant="tertiary">
              <Link href={`/bookings/${deposit.bookingReference}`}>
            ...
            {deposit.release ? (
              <Button asChild variant="tertiary">
                <Link href={`/deposits/${deposit.bookingReference}/statement`}>
      ...
      <SectionCard id="history-heading" title="History" className="mt-xl">
        <DepositHistory
          history={history}
          path={`/deposits/${encodeURIComponent(reference)}`}
          actorNames={actorNames}
        />
      </SectionCard>
    </div>
    ...
    >
      ...
      {isShort ? (
        <p className="mt-md text-body-sm text-copy">
          ...
          be checked in —{' '}
          <Link
            href={`/bookings/${encodeURIComponent(deposit.bookingReference)}`}
