  ...
  return (
    <TableRow interactive className="group">
      <TableCell className="font-mono text-foreground tabular-nums">
        <TableRowLink href={`/deposits/${encodeURIComponent(deposit.bookingReference)}`}>
...
function LedgerEmptyState({ view, isFiltered }: { view: LedgerView; isFiltered: boolean }) {
      ...
      <EmptyState
        ...
        action={
          <Button asChild variant="tertiary">
            <Link href="/deposits">Clear filters</Link>
          </Button>
        }
      />
    )
  ...
  return (
    <EmptyState
      ...
      action={
        <Button asChild variant="tertiary">
          <Link href="/deposits">Show everything held</Link>
        </Button>
      }
    />
  )
