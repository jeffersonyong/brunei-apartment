...
export default async function UnitsPage({ searchParams }: PageProps) {
  ...
  // different permission on a different screen; see /settings/units.
        ...
        {visible.length === 0 ? (
          <EmptyState
            ...
            action={
              isFiltered ? (
                <Button asChild variant="tertiary">
                  <Link href="/units">Clear filters</Link>
                </Button>
              ) : mayEditRegistry ? (
                <Button asChild variant="tertiary">
                  <Link href="/settings/units">Set up the units</Link>
                </Button>
              ) : undefined
            }
          />
  ...
  return (
    <TableRow interactive className="group">
      <TableCell className="font-mono text-foreground tabular-nums">
        <TableRowLink href={`/units/${encodeURIComponent(unit.ref)}`}>{unit.ref}</TableRowLink>
      </TableCell>
