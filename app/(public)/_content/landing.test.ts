...
import { facilityCopy, unitTypeCopy } from './landing'
 ...
 *
 * Both the unit-type and the facility cards are built from Property settings
 * now, not from this module (17 September 2026), so what is checked here is
 * that each line of copy is filed under a key a real row could carry — a
 * typo'd key silently costs a card its description and its photograph.
 */
describe('the landing cards', () => {
  test.each(Object.keys(facilityCopy))('the %s copy is filed under a usable slug', (slug) => {
    expect(parsePlacementKey(`facility:${slug}`)).toEqual({ kind: 'facility', slug })
  })

  test('every facility with copy has both a line and a photograph label', () => {
    for (const copy of Object.values(facilityCopy)) {
